...
export default function OfficerLayout() {
  ...
  const [counts, setCounts] = useState({ pending: 0, assigned: 0, rejected: 0 });
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        const assignedCount = allReports.filter(r => r.status === 'Assigned').length;
      ...
      <div className="flex flex-1 flex-col lg:flex-row">
        ...
        >
          <div className="flex flex-1 flex-col gap-6 border-r bg-card/80 p-6 backdrop-blur">
            <div>
              <h2 className="text-lg font-semibold text-foreground">Officer Dashboard</h2>
