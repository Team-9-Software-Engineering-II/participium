/**
 * @file Integration Test for Reports API (Public Relations Officer Flow)
 * @description Tests the authorization, review actions (reject/assign),
 * and state conflicts for the municipal reports endpoint.
 */
...
/** @type {string} Cookie for the authorized Public Relations Officer. */
let prOfficerCookie;
...
/**
 * @description Sets up the database and logs in both PR Officer and Citizen.
 */
beforeAll(async () => {
  // Setup the database and run seeders
  await setupTestDatabase();

  // 1. Log in PR Officer (Authorized User) using utility
  prOfficerCookie = await loginAndGetCookie(prOfficerLogin);

  // 2. Log in Standard Citizen (Unauthorized User) using utility
...
describe("Reports API Integration (Public Relations Officer Flow)", () => {
  ...
  describe(`PUT ${BASE_URL}/:reportId (Reject Flow)`, () => {
    ...
    it("should successfully reject the report and save the mandatory reason (200)", async () => {
      ...
      expect(reportInDb.technicalOfficerId).toBeNull();
    });
  ...
  describe("State Conflict and Advanced Validation", () => {
    ...
    it("should return 400 Bad Request for an invalid or unknown action type in payload", async () => {
      ...
      const res = await request(app)
        .put(pendingReportUrl)
        .set("Cookie", prOfficerCookie)
        .send({ action: "completed_by_magic" });
